...
if not eF.layoutEventHandler then
    eF.layoutEventHandler =
        CreateFrame("Frame", "ElFramoOnRosterUpdateFrame", UIParent)
end
eF.layoutEventHandler:RegisterEvent("GROUP_ROSTER_UPDATE")
eF.layoutEventHandler:RegisterEvent("PLAYER_REGEN_DISABLED")
local ipairs = ipairs
function eF.layoutEventHandler:handleEvent(event, ...)
    ...
    for _, frame in ipairs(all_frames) do
        ...
        if (name ~= frame.name) then
            frame:updateUnit(true)
        end
...
local post_combat_functions = {
    ...
    ["updateFrameSizes"] = function()
        for k, v in pairs(eF.list_all_active_unit_frames()) do
            if v.flagged_post_combat_size_update then
                v:updateSize()
            end
        end
    end,
...
function eF.loadingFrame:handleEvent(event, ID)
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        if
            (instanceName ~= eF.info.instanceName) or
                (instanceID ~= eF.info.instanceID)
         then
...
if not eF.caster_watcher_frame then
    eF.caster_watcher_frame =
        CreateFrame("Frame", "ElFramoCastWatcher", UIParent)
end
...
local recorded_casts_end, recorded_casts_start, doAfter =
    eF.recorded_casts_end,
    eF.recorded_casts_start,
    C_Timer.After
local function remove_cast_timer(castID, sourceGUID, castEnd)
    local castID = castID or "None"
    if recorded_casts_end[castID] then
        return
    end
    ...
    doAfter(
        ...
        function()
            if
                (not casts[sourceGUID]) or not (casts[sourceGUID][7] == castID) or
                    not (targets[sourceGUID]) or
                    (#targets[sourceGUID] == 0)
             then
                return
            end
...
function eF.caster_watcher_frame:handleEvent(event, sourceUnit, castID)
    ...
    elseif
     ...
     then
        ...
        if castID ~= casts[sourceGUID][7] then
            return
        end
        ...
        if castID then
            recorded_casts_end[castID] = true
        end
...
eF.caster_watcher_frame:SetScript(
    "OnEvent",
    eF.caster_watcher_frame.handleEvent
)
...
for i, v in ipairs(chat_events) do
    eF.chat_watcher_frame:RegisterEvent(v)
end
...
if not eF.rc_watcher_frame then
    eF.rc_watcher_frame =
        CreateFrame("Frame", "ElFramoReadyCheckWatcher", UIParent)
end
for i, v in ipairs(rc_events) do
    eF.rc_watcher_frame:RegisterEvent(v)
end
