...
local iconApplySmartIcon,
	iconUpdateCDWheel,
	iconUpdateTextTypeT,
	iconUpdateText2TypeT,
	iconUpdateTextTypeS,
	iconUpdateText2TypeS
...
local function is_aura_new(self, count, expirationTime, spellID)
	...
	return not ((count == aI.count) and (expirationTime == aI.expirationTime) and
		(spellID == aI.spellID))
end

function taskFuncs:auraExtras(
	name,
	icon,
	count,
	debuffType,
...
function taskFuncs:applyMsgAdopt(msg, event)
	...
	if bool and bool2 then
		...
		aI.duration =
			(self.para.match_duration_in_message and (aI.count ~= 0) and
			min(aI.count, 60)) or
			self.para.chatTimed
...
function taskFuncs:applyReadyCheck(status, ended, time)
	...
	if not bool and self.filled then
		self:disable()
	end
end
...
function taskFuncs:rcOnUpdate()
	local t, eT = GetTime(), self.auraInfo.expirationTime
	if t > eT + (self.para.rcLinger or 0) then
		self:disable()
	end
end
...
function taskFuncs:applyListAuraAdopt(unit)
	...
	if active == 0 and self.filled then
		self:disable()
	end
	if active > 0 then
		if not self.filled then
		...
		for i = 1, active do
			if not self[i].filled then
				self[i]:enable()
			end
		end
		for i = active + 1, self.count do
			if self[i].filled then
				self[i]:disable()
			end
		end
	end
...
local function is_target_of(unit, k)
	...
	for i = 1, #targets do
		if UnitIsUnit(unit, targets[1].id or "") then
			return true
		end
	end
	return false
...
function taskFuncs:iconUpdateVisibilityTrem()
	if not self.auraInfo.expirationTime then
	...
	if (trem < (max or 0)) and (trem > (min or 0)) then
		self:setVisibility(1)
	else
		self:setVisibility(0)
	end
...
function taskFuncs:iconAdoptAuraByNameWhitelist(name, _, _, _, dur)
	if self.para.ignorePermanents and dur == 0 then
		return false
	end
	if (self.para.iDA or 0 > 0) and dur > self.para.iDA then
		return false
	end
...
function taskFuncs:iconApplySmartIcon()
	if self.isListElement then
		for i = 1, self.active do
			iconApplySmartIcon(self[i])
		end
		return
	end

	if not (self.filled and self.auraInfo.new_aura) then
		return
	end
...
function taskFuncs:changeDispellableDebuffSize()
	if self.isListElement then
		for i = 1, self.active do
			changeDispellableDebuffSize(self[i])
		end
		return
	end
	if not (self.filled and self.auraInfo.new_aura) then
	...
	if not self.auraInfo.debuffType then
		return
	end
	if eF.isDispellable[self.auraInfo.debuffType] then
		self:SetSize(
			self.para.dispellableHeight or self.para.height,
			self.para.dispellableWidth or self.para.width
		)
	else
...
local function dispellable_compare(v1, v2)
	return ((v1.dispellable == v2.dispellable) and v1.index < v2.index) or
		(v1.dispellable and (not v2.dispellable))
end

local help_table1 = {}
function taskFuncs:orderByDispellable()
	if not self.isListElement then
		return
	end
	wipe(help_table1)
	local bool = false
	for i = 1, self.active do
		...
		tbl.dispellable =
			(tbl.debuffType and eF.isDispellable[tbl.debuffType]) or false
		if (not bool) and tbl.dispellable then
			bool = true
		end
		tbl.index = i
	end
	...
	if not bool then
		return
	end --if no dispellable aura, just leave

	table_sort(help_table1, dispellable_compare)

	for i = 1, self.active do
		self[i].auraInfo = help_table1[i]
		if i ~= help_table1[i].index then
			help_table1[i].new_aura = true
		end
	end
end
...
function taskFuncs:iconUpdateCDWheel()
	if self.isListElement then
		for i = 1, self.active do
			iconUpdateCDWheel(self[i])
		end
		return
	end

	local aI = self.auraInfo
	if not (self.filled and aI.new_aura) then
		return
	end
...
function taskFuncs:iconUpdateTextTypeT()
	if self.isListElement then
		for i = 1, self.active do
			iconUpdateTextTypeT(self[i])
		end
		return
	end
	local t, aI = GetTime(), self.auraInfo
	local s =
		(aI.isPermanent and "") or self.textDecimalFunc(aI.expirationTime - t)
	self.text:SetText(s)
end
...
function taskFuncs:iconUpdateText2TypeT()
	if self.isListElement then
		for i = 1, self.active do
			iconUpdateText2TypeT(self[i])
		end
		return
	end
	local t, aI = GetTime(), self.auraInfo
	local s =
		(aI.isPermanent and "") or self.text2DecimalFunc(aI.expirationTime - t)
...
function taskFuncs:updateBorderColorDebuffType()
	if self.isListElement then
		for i = 1, self.active do
			updateBorderColorDebuffType(self[i])
		end
		return
	end
	if not self.filled then
