...
tabs:SetLayout("Flow")
tabs:SetTabs({{text="Layouts", value="layouts"}, {text="Elements", value="elements"}, {text="Profiles", value="profiles"}})
...
eF.interface_onUpdate_frame=CreateFrame("Frame","ElFramoInterfaceOnUpdateFrame",UIParent)
...
function eF.interface_main_frame:onClose()
	...
	if not (next(self.element_tests)==nil) then eF:refresh_visible_unit_frames() end --check if table is not empty
...
--RANDOM AURAS
local random_aura_names={"Rejuvenation","Atonement","Big dumb","Shadow Word: Pain","Riptide"}
