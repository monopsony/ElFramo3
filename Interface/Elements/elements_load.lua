...
local function hidden_function(index)
    ...
    local bool =
        (eF.para.elements[name].load.loadAlways or
        eF.para.elements[name].load.loadNever)
    if not index then
        return bool
    else
...
local function set_multiline_parameter(index, text)
    ...
    if type(eF.para.elements[name][index]) == "list" then
        wipe(eF.para.elements[name][index])
    end
    local para, s = {}, ""
    for k, v in pairs({strsplit(delimiters, text)}) do
        if v and (not isWhiteSpaces(v)) then
            para[v] = true
        end
    end
...
do
    ...
    args["instances_list_prot"] = {
        ...
        order = 42,
        desc = function()
            return eF.interface_list_help_tooltip("instance names/IDs")
        end,
        width = "full",
        name = "Instance name/ID whitelist",
        hidden = function()
            return hidden_function(5)
        end,
        multiline = true,
        set = function(self, value)
...
-- layouts
do
    args["showLayoutHeader_prot"] = {
        name = "Layout",
        type = "header",
        order = 50
    }

    args["showLayoutAny_prot"] = {
        name = highlight_colour .. "Any",
        type = "toggle",
        order = 51,
        set = function(self, value)
            set_current_parameter(7, "loadAlways", value)
        end,
        disabled = function()
            return hidden_function()
        end,
        get = function(self)
            return get_current_parameter(7, "loadAlways")
        end
    }

    args["showUnitClasses_prot"] = {
        type = "multiselect",
        name = "Show when layout is",
        order = 53,
        values = function()
            local p = eF.para.layouts
            local t = {}
        ...
        set = function(self, key, value)
            set_current_parameter(7, key, value)
        end,
        hidden = function()
            return hidden_function(7)
        end,
        get = function(self, key)
            return get_current_parameter(7, key)
        end
    }
end
