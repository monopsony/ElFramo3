...
local orient_to_anchors = {
    [1] = {"TOPLEFT", "BOTTOMLEFT"},
...
local function generate_header_anchor(g1, g2)
    local header_anchor
    if (g1 == "up" or g2 == "up") then
        header_anchor = "BOTTOM"
    elseif (g1 == "down" or g2 == "down") then
        header_anchor = "TOP"
    end

    if (g1 == "right" or g2 == "right") then
        header_anchor = header_anchor .. "LEFT"
    elseif (g1 == "left" or g2 == "left") then
        header_anchor = header_anchor .. "RIGHT"
    end
...
function layout_methods:checkVisibility()
    ...
    -- classes
    if bool then
        if not (classes.Any or classes[eF.info.playerClass]) then
            bool = false
        end
    end
    if not bool then
...
function eF:register_new_layout(key)
    ...
    if not eF.para.layouts[index] then
        ...
        eF.para.layouts[index]["parameters"] =
            eF.table_deep_copy(header_default_parameters)
...
function eF:apply_layout_para_index(index)
    ...
    -- set order defaults
    if header.att.groupBy == "CLASS" then
        att.groupingOrder = default_class_order
    elseif header.att.groupBy == "GROUP" then
        att.groupingOrder = default_group_order
    elseif header.att.groupBy == "ROLE" then
        att.gorupingOrder = default_role_order
    end
